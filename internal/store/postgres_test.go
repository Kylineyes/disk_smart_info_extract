...
func TestPostgresCreateIndexSQL(t *testing.T) {
	...
	want := `CREATE UNIQUE INDEX IF NOT EXISTS "uq_smart_log_date_model_serial" ON "smart_log" (snapshot_date, model, serial)`
	if sql != want {
...
func TestPostgresUpsertArguments(t *testing.T) {
	...
	if got, want := len(arguments), 39; got != want {
		t.Fatalf("argument count = %d, want %d", got, want)
	}
	if got, want := arguments[9], maxCounter.String(); got != want {
		t.Errorf("capacity argument = %#v, want %q", got, want)
	}
	if got, want := arguments[17], value; got != want {
		t.Errorf("temperature argument = %#v, want %d", got, want)
	}
	if got, want := arguments[21], maxCounter.String(); got != want {
		t.Errorf("data units read argument = %#v, want %q", got, want)
	}
	if arguments[18] != nil {
		t.Errorf("missing available spare argument = %#v, want nil", arguments[18])
	}
}
...
func TestValidateSmartLog(t *testing.T) {
	tests := []struct {
		...
		{"missing source", func(log *model.SmartLog) { log.SourceFile = "" }},
		{"missing raw log", func(log *model.SmartLog) { log.RawLog = "" }},
	}
...
func validSmartLog() model.SmartLog {
	return model.SmartLog{
		...
		SourceFile:   "/logs/20260831.log",
		Device: model.DeviceInfo{
			Model:  "NVMe Example",
			Serial: "SERIAL1234",
		},
		Health: model.HealthInfo{OverallHealth: "PASSED"},
		RawLog: "smartctl output",
	}
}
